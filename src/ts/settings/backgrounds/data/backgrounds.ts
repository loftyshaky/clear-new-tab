...
class Class {
    ...
    public thumbnail_src = ({
        ...
        err(() => {
            const fallback: string | undefined = n(
                ...
                ? d_backgrounds.Cache.background_thumbnail_cache_items[id].thumbnail
    ...
    public get_background_thumbnail_by_id = ({
        ...
        err_async(async () => {
            const background_thumbnail: i_db.BackgroundThumbnail | undefined = n(id)
                ? await s_db.Manipulation.get_background_thumbnail({ id })
                : undefined;
    ...
    public set_backgrounds = ({
        ...
        err_async(async () => {
            ...
            backgrounds_2.map((background: i_db.Background): void =>
                err(() => {
                    d_backgrounds.BackgroundAnimation.push_already_animated_id({
                        id: background.id,
                    });
                }, 'cnt_1127'),
            );
            ...
            runInAction(() =>
                err(() => {
                    this.backgrounds = s_i.I.sort_by_i_ascending({
                        data: backgrounds_2,
                    }) as i_db.Background[];
                }, 'cnt_1128'),
            );
    ...
    public get_img_i = ({ i }: { i: number }): number =>
        err(() => d_pagination.Page.offset + i + 1, 'cnt_1449');
}
