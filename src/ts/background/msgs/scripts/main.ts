...
we.runtime.onMessage.addListener((msg: t.Msg): any =>
    err(() => {
        ...
        if (msg_str === 'push_tab_id') {
            return s_tabs.TabIds.i()
                .push_tab_id()
                .then(() => s_tabs.TabIds.i().set_last_visited_new_tab_id())
                .then(() => true)
        ...
        if (msg_str === 'get_installed') {
            return s_browser_theme.ThemeId.i()
                .get_installed()
        ...
        if (msg_str === 'uploading_theme_background_now') {
            return Promise.resolve(s_browser_theme.Main.i().getting_theme_background);
        }

        if (msg_str === 'clear_slideshow_timer') {
            return s_background.BackgroundChange.i()
                .clear_slideshow_timer()
                .then(() => true)
        ...
        if (msg_str === 'schedule_background_display') {
            return s_scheduler.Main.i()
                .schedule_background_display()
                .then(() => true)
                .catch((error_obj: any) => show_err_ribbon(error_obj, 'cnt_1488'));
        }

        if (msg_str === 'open_default_new_tab_page') {
            s_home_btn.Main.i().open_default_new_tab_page();
