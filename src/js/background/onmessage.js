...
const get_img_i_by_id = id => {
    return shared_b.mut.imgs.findIndex(img => img.id == id);
}
//< get_img_i_by_id f

//> get_img_id_by_i f
const get_img_id_by_i = i => {
    return shared_b.mut.imgs[i].id;
}
//< get_img_id_by_i f
...
browser.runtime.onMessage.addListener((message, sender, send_response) => {
    ...
    if (msg == 'get_img') { // set, preload images and get current image from new tab
        ...
        send_response({ next_img_after_last_visible_img_id: next_img_after_last_visible_img_id });
        ...
        shared_b.get_installed_theme_id()
            .then(theme_id => {
                return theme_img.get_theme_img(theme_id, message.reinstall_even_if_theme_img_already_exist);
        ...
        send_response({ img_id_before_drop: img_id_before_drop, ids_of_imgs_to_move: ids_of_imgs_to_move });
