...
const get_first_or_last_visible_item = (mode, hovered_tree) => {
    ...
    } else if (mode == 'new_tab' || mode == 'new_tab_bookmarks_bar') {
...
//>1 append_drop_area f
const append_drop_area = (mode, hovered_item, item_to_move, drop_area, first_item, second_item, traverse_1, traverse_2, append_f, direction, client_key, e) => {
...
const drop_options = async () => {
    try {
        ...
        const response = await x.send_message_to_background_c({ message: "get_ids_of_imgs_to_shift", move_type: move_type, img_i_before_drop: img_i_before_drop, start_i: start_i, end_i: end_i });
